...
func TestChannelMetricsResolvesFollowerAndSubscriberCounts(t *testing.T) {
	...
	model.services.followerLookup = &appFakeFollowerLookup{page: twitch.FollowersPage{Total: 42, Followers: []twitch.Follower{
		{UserID: "1", UserName: "Viewer1"},
	}}}
	model.services.subscriptionLookup = &appFakeSubscriptionLookup{page: twitch.SubscriptionsPage{Total: 7, Points: 9}}
	...
	if !model.metrics.followerCountKnown || model.metrics.followerCount != 42 {
		t.Fatalf("followerCount = %d known=%v, want 42/true", model.metrics.followerCount, model.metrics.followerCountKnown)
	}
	if !model.metrics.subscriberCountKnown || model.metrics.subscriberCount != 7 {
		t.Fatalf("subscriberCount = %d known=%v, want 7/true", model.metrics.subscriberCount, model.metrics.subscriberCountKnown)
	}
...
func TestChannelMetricsErrorLeavesCountsUnknown(t *testing.T) {
	model := newMockModel("example", config.Default())
	model.selfBroadcasterID = "123"
	model.services.followerLookup = &appFakeFollowerLookup{err: errors.New("twitch says no")}
	model.services.subscriptionLookup = &appFakeSubscriptionLookup{err: errors.New("twitch says no")}
...
func TestChannelMetricsTickReschedulesAndResolvesAgain(t *testing.T) {
	model := newMockModel("example", config.Default())
	model.selfBroadcasterID = "123"
	model.services.followerLookup = &appFakeFollowerLookup{page: twitch.FollowersPage{Total: 1}}

	cmd := model.scheduleChannelMetricsTick()
	if cmd == nil {
		t.Fatal("scheduleChannelMetricsTick returned nil, want a tick command")
	}
	if !model.metrics.channelMetricsTickScheduled {
