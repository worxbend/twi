...
const (
	// maxOAuthResponseBodySize bounds an OAuth token response, which is a
	// handful of short fields. Matches internal/auth's identical constant.
	maxOAuthResponseBodySize = 4096
	// maxHelixResponseBodySize bounds a Helix API response. The largest of
	// them (global emote and badge sets) are a few hundred kilobytes, so 4
	// MiB leaves generous headroom.
	maxHelixResponseBodySize = 4 << 20
)
