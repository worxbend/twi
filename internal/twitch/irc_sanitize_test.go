...
func TestSanitizeIRCTextNeutralizesCommandInjection(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			got := sanitizeIRCText(tc.text)
			if strings.ContainsAny(got, "\r\n") {
				t.Fatalf("sanitizeIRCText(%q) = %q, still contains a line break", tc.text, got)
			}
		})
	}
}
...
func TestSanitizeIRCTextKeepsVisibleContent(t *testing.T) {
	got := sanitizeIRCText("hello\r\nworld")
	if want := "hello  world"; got != want {
		t.Fatalf("sanitizeIRCText = %q, want %q", got, want)
	}
}

func TestSanitizeIRCTextDropsControlsButKeepsCTCP(t *testing.T) {
	got := sanitizeIRCText("\x01ACTION wa\x07ves\x1b[31m\x01")
...
func TestSanitizeIRCTextPreservesUnicode(t *testing.T) {
	...
	if got := sanitizeIRCText(text); got != text {
		t.Fatalf("sanitizeIRCText(%q) = %q, want it unchanged", text, got)
	}
}

func TestSanitizeIRCTextTruncatesToTwitchLimit(t *testing.T) {
	got := sanitizeIRCText(strings.Repeat("か", 600))
	if runes := []rune(got); len(runes) != MaxChatMessageRunes {
		t.Fatalf("len(runes) = %d, want %d", len(runes), MaxChatMessageRunes)
	}
}
...
func TestSanitizeIRCTextTruncationKeepsActionClosed(t *testing.T) {
	got := sanitizeIRCText("\x01ACTION " + strings.Repeat("x", 600) + "\x01")
	runes := []rune(got)
