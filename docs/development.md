...
- `internal/storage` defines the context-aware asset cache boundary, an in-memory test cache, a disk-backed asset cache for metadata, public source URLs, URL-free payload identities, and bytes under the platform cache directory, TTL/size pruning, and filesystem probes used by diagnostics.
