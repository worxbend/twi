...
non-secret config and hand off to login; refreshed IRC tokens are saved through
the supported credential store when available. Windows Credential Manager
persistence, credentialed Twitch release validation, exact Docker CLI
validation, and manual Kitty/Ghostty image validation remain planned or
...
| RR-016 | Stored credential files are created or reused with unsafe permissions. | Medium | High | Keep credential persistence behind `CredentialStore`; support the file fallback only on Unix builds; require exact `0700` credential directories, `0600` credential files, symlink rejection, and no-follow opens; never enable a Windows credential file; implement Windows persistence through native Credential Manager before claiming Windows saved credentials; keep other non-Unix targets deferred until a backend is selected. | Twitch integration engineer and QA/release engineer | Storage permission unit tests; non-Unix gated tests; Windows Credential Manager adapter tests; `config show`/`doctor` redaction checks; manual docs review. |
