...
| Login/setup | Partial | `twi setup` writes non-secret config values and can hand off to login. On supported Unix builds, `twi login` runs the browser/local-callback OAuth flow or a `--dry-run` explanation, validates returned tokens, and saves them without printing them through the restrictive credential-file fallback. Non-Unix builds use env/config credentials. |
